...
public class SearchAlgo2 extends PApplet
{
    GDropList list;
    String items[] = {"~Pick One~","Beverages","Condiments","Produce","Meat/Poultry","Seafood","Dairy Products",
    "Confections","Grains/Cereals"};
    public static String product[]=new String[50];
    public static String category[]=new String[50];
    public static double price[]=new double[50];
    GTextArea txtresults;
	public void setup()
	{
		size(640, 480, JAVA2D);
                list = new GDropList(this,10,10,100,200,9);
                list.setItems(items, 0);
                txtresults=new GTextArea(this,130,10,400,300,1);
	...
	public void handleDropListEvents(GDropList list, GEvent event) 
	{
                ...
                String result[] = search(items[cat]);
                for (int x=0; x<result.length; x++){
                    txtresults.appendText(result[x]);
                }
	...
	public void draw()
	{
        ...
        public int getNumItems(String item){
            ...
            for(int x=0; x<category.length;x++){
                if(category[x].equals(item)) total++;
            }
            return total;
        }
        
        public String[] search (String item){
            int size = getNumItems(item);
            String results[] = new String[size];
            int counter=0;
            for(int x=0;x<category.length;x++){
                if (item.equals(category[x])){
                    results[counter] = price[x] + "    " + product[x];
                    counter++;
                }
            }
            return results;
